...
| 项目类型 | 特征 | 首个会话做什么 |
|----------|------|---------------|
| **空白项目** | 无 Git 仓库，无 init.sh/progress.txt/features.json | 先需求定义，再初始化 |
| **已有项目** | 有 Git 仓库，已有基础配置文件 | 直接开始功能开发 |

### 空白项目路径

```
需求定义 → 创建文件 → 初始化仓库 → 环境就绪 → 功能开发 → 提交
```

1. **需求定义**：与用户沟通项目目标、技术栈、功能规划
2. **创建文件**：生成 features.json、progress.txt、init.sh
3. **初始化仓库**：`git init` 并提交初始版本
4. **环境就绪**：执行 init.sh + 冒烟测试
5. **功能开发**：按 priority 领取任务实现
6. **提交**：更新状态文件 + git commit

### 已有项目路径

```
检查环境 → 读取进度 → 领取任务 → 功能开发 → 提交
```

1. **检查环境**：确认 init.sh、progress.txt、features.json 存在
2. **读取进度**：查看 progress.txt 历史记录
git log 查看提交历史
3. **领取任务**：从 features.json 找到 passes=false 的最高优先级功能
4. **环境就绪**：执行 init.sh + 冒烟测试
5. **功能开发**：实现功能
6. **提交**：更新状态文件 + git commit
---
